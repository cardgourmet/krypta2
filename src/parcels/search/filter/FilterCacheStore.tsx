...
import { fetchDlcFilters, fetchDlcFiltersValues, fetchDlcFilterValues } from '@/parcels/tcg/dlc/api.ts';
import { fetchMtgFilters, fetchMtgFiltersValues, fetchMtgFilterValues } from '@/parcels/tcg/mtg/api.ts';
import { fetchPcgFilters, fetchPcgFiltersValues, fetchPcgFilterValues } from '@/parcels/tcg/pcg/api.ts';
import type {
...
export const useFilterCacheStore = create<FilterCacheStore>((set, get) => ({
  ...
  findOrFetchValues: async (
    ...
    for (const keyword of keywords) {
      if (dynamicFilterKeywords.includes(keyword.toLowerCase())) {
        isDynamic = true;
      }
    ...
    if (isDynamic) {
      ...
      let res: GourmetApiResponse<SearchQueryExecutorFilterValues> | undefined;
      if (tcg === 'mtg') {
        res = await fetchMtgFilterValues(keyword, abort, operator as TcgFilterOperator, currentValue, 64);
      } else if (tcg === 'pcg') {
        res = await fetchPcgFilterValues(keyword, abort, operator as TcgFilterOperator, currentValue, 64);
      } else if (tcg === 'dlc') {
        res = await fetchDlcFilterValues(keyword, abort, operator as TcgFilterOperator, currentValue, 64);
      }
      if (!res || res?.error || !res.data) return { error: res?.error };

      foundValuesByKeyword[keyword] = res.data.values;
      return { data: foundValuesByKeyword };
    }

    let res: GourmetApiResponse<Record<string, SearchQueryExecutorFilterValues>> | undefined;
    if (tcg === 'mtg') {
      res = await fetchMtgFiltersValues(missingKeywords, abort);
    } else if (tcg === 'pcg') {
      res = await fetchPcgFiltersValues(missingKeywords, abort);
    } else if (tcg === 'dlc') {
      res = await fetchDlcFiltersValues(missingKeywords, abort);
    }
    if (!res || res?.error || !res.data) return { error: res?.error };
