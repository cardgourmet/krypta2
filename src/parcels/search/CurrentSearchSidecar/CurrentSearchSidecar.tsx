import { ActionIcon, Group, Overlay, ScrollArea, SimpleGrid } from '@mantine/core';
...
import { GourmetText } from '../../generic/mantine/GourmetText';
import { Typeset } from '../../generic/Typeset/Typeset';
import { type CardProperties, createProps } from '../../overview/cards/CardGrid/CardGridEntry/createProps';
import useCardOverviewData from '../../overview/cards/CardOverview/useCardOverviewData';
import { useWorkContextReloader } from '../../overview/cards/CardOverview/useWorkContextReloader';
import { ImageCard } from '../../overview/cards/ImageCard/ImageCard';
import { useOverviewWorkMenuStore, useOverviewWorkStore } from '../../selection/useOverviewWorkStore';
import type { TcgSearchDataCard } from '../../tcg/types';
import type { Tcg } from '../../tcg/useTcgByLocation';
import styles from './CurrentSearchSidecar.module.css';

export const CurrentSearchSidecar = () => {
  ...
  return (
    <div className={styles.base}>
      <div>
        <Typeset asChild size="lg" style={{ fontFamily: 'var(--cgm-title-font-family)' }} weight={600}>
          <h2>{t('title')}</h2>
        </Typeset>

        <Typeset block>
          {!!workMeta?.other?.set && workMeta?.other?.set?.translations.en.name}
          {!workMeta?.other?.set && workMeta?.other?.querySettings?.query && workMeta?.other?.querySettings?.query}
        </Typeset>
      </div>
      ...
      <ScrollArea.Autosize h="100%" mah="100%" mx="-1rem">
        <SimpleGrid cols={2} px="1rem" spacing="0.5rem">
